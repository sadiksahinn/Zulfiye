...
export default function AccountingPage() {
  const [rentals, setRentals] = useState<RentalAccounting[]>([]);
  ...
  async function loadAccounting() {
    ...
    setRentals((data || []) as RentalAccounting[]);
    setLoading(false);
  }
  ...
  const filteredRentals = useMemo(() => {
    ...
    return rentals.filter((item) =>
      [item.customer_name, item.product_name, item.status, item.delivery_date, item.return_date, item.notes]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(q)
  ...
  const totals = useMemo(() => {
    return rentals.reduce(
      (acc, item) => {
        ...
        const remaining = Number(item.remaining_amount ?? Math.max(total - deposit, 0));
      ...
      <div className="space-y-5 lg:space-y-8">
        <div className="overflow-hidden rounded-[1.6rem] border border-white/70 bg-gradient-to-br from-[#211b16] via-[#2b231c] to-[#b69463] p-5 text-white shadow-[0_18px_55px_rgba(33,27,22,.16)] lg:rounded-[2rem] lg:p-7">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/70">
            Kiralama tutarları, alınan ödemeler ve kalan bakiyeleri tek panelden takip edin.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3 lg:gap-6">
          <StatCard title="Ciro" value={formatMoney(totals.total)} icon={<TrendingUp size={21} />} />
          <StatCard title="Tahsilat" value={formatMoney(totals.deposit)} icon={<CreditCard size={21} />} />
        ...
        <section className="premium-card p-4 lg:p-7">
          <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="premium-muted text-sm">Kiralama bazlı ödeme ve kalan bakiye takibi</p>
            </div>
          ...
          {message ? (
            <div className="rounded-2xl border border-[#eadfce] bg-white/70 p-4 text-sm font-bold text-[#6d6256]">{message}</div>
          ) : null}
          ...
          <div className="space-y-3">
            {loading ? (
              ...
              <EmptyState text="Henüz ödeme kaydı bulunmuyor." />
            ) : (
              filteredRentals.map((item) => {
                ...
                return (
                  <div key={item.id} className="rounded-[1.4rem] border border-[#eadfce] bg-white/65 p-4 shadow-sm transition hover:bg-white/85">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      <div>
                        <h3 className="text-base font-black text-[#211b16]">
                          {item.customer_name || "Müşteri bilgisi yok"}
                        </h3>
                        <p className="mt-1 text-xs font-bold text-[#8a7f72]">
                          {[item.product_name, item.delivery_date, item.return_date].filter(Boolean).join(" • ") || "Kiralama detayı yok"}
                        </p>
                      </div>

                      <div className="grid grid-cols-3 gap-2 text-right lg:min-w-[360px]">
                        <MoneyBox label="Toplam" value={total} />
                        <MoneyBox label="Alınan" value={deposit} />
  ...
  return (
    <div className={`premium-card p-5 transition hover:-translate-y-0.5 lg:p-7 ${dark ? "bg-[#211b16] text-white" : ""}`}>
      <div className="flex items-center justify-between gap-4">
        ...
        <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${dark ? "bg-white/10 text-[#d8bd84]" : "bg-[#b69463]/15 text-[#b69463]"}`}>
          {icon}
        </div>
      </div>
    </div>
